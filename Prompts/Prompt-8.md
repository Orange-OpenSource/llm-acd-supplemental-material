...
You will also receive the reward obtained after the last action. This reward reflects the effectiveness of your previous decision.
The reward will be negative or zero. If, after your action, a machine is controlled by the attacker, a negative reward will be given. The more machines controlled by the attacker, the lower the reward will be. Not all machines have the same impact on the reward: for example, if the server is controlled by the attacker, the reward will be -10, whereas if a user machine is controlled, the reward will be -1, since the server is more critical.
...
3. **If the last action was Remove on Op_Server0 and it Failed:**
   - If the last action was `take_action(Remove, Op_Server0)` and the action result is Failed:
     `take_action(Restore, Op_Server0)`

4. **If any Enterprise or Defender target is compromised at the Privileged or User level:**
...
5. **If any Enterprise or Defender target is being exploited:**
   - For any target labeled as Enterprise or Defender, if (target, Exploit, compromised):
     `take_action(Remove, target)`

6. **If the last action was Remove on an Enterprise or Defender target and it Failed:**
   - If the last action was `take_action(Remove, target)` where target is any Enterprise or Defender, and the action result is Failed:
     `take_action(Restore, target)`

7. **If a decoy is available for Enterprise0:**
   - If nb_decoys > 0 for Enterprise0:
     `take_action(Decoy, Enterprise0)`

8. **If a decoy is available for any Enterprise or Defender target:**
   - For any target labeled as Enterprise or Defender, if nb_decoys > 0:
     `take_action(Decoy, target)`

9. **If a decoy is available for Op_Server0:**
   - If nb_decoys > 0 for Op_Server0:
     `take_action(Decoy, Op_Server0)`

10. **If any User target is being exploited:**
   - For any target labeled as User, if (target, Exploit, compromised):
     `take_action(Remove, target)`

11. **If any User target is being scanned:**
   - For any target labeled as User, if (target, Scan, compromised):
     `take_action(Remove, target)`

12. **Never use the Restore action on any target labeled as User or Op_Host.**

13. **If no other action is appropriate, use the Monitor action by default:**
    `take_action(Monitor,)`
